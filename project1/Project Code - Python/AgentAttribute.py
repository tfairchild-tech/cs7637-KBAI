...
class Attribute:
    ...
    def __init__(self, dictionary):
        ...
        if len(dictionary):
            ...
            #logging.debug("initializing dictionary: %s", dictionary)
            ...
            #    else:
            #        self.inside = 'None'
                self.inside = dictionary.get('inside', 'None')
            except KeyError:
                pass
            ...
            try:
                self.leftof = dictionary.get('left-of',999)
            except KeyError:
                pass
            ...
            try:
                self.overlaps = dictionary.get('overlaps',999)
            except KeyError:
                pass
    ...
    def set_inside(self, i):
        #TODO: DO I WANT TO SET THIS AS NONE?
        #if i != 'None':
        #    i_list = [x.strip() for x in i.split(',')]
    ...
    def get_attributes_str(self):
        return "%s,%s,%s,%s,%s,%s,%s,%s" % (self.shape,self.size,self.angle,self.fill,self.inside,self.width,self.length,self.overlaps)

    # get_attributes
    def get_attributes(self):
        return self.shape,self.size,self.angle,self.fill,self.inside,self.width,self.length,self.overlaps
    ...
    def get_attribute_dict(self):
        ...
        try:
            thisDict['overlaps'] = self.get_overlaps()
        except:
            AttributeError
    ...
    def the_same(self, attrA,attrB):
        #logging.debug("in Attribute.the_same()... ")
        #logging.debug("shape %s", attrA.get_shape())
        #logging.debug("shape %s", attrB.get_shape())

        #logging.debug("size %s", attrA.get_size())
        #logging.debug("size %s", attrB.get_size())

        #logging.debug("fill %s", attrA.get_fill())
        #logging.debug("fill %s", attrB.get_fill())

        #logging.debug("left of %s", attrA.get_left_of())
        #logging.debug("left of%s", attrB.get_left_of())

        #logging.debug("width %s", attrA.get_width())
        #logging.debug("width %s", attrB.get_width())

        #logging.debug("length %s", attrA.get_length())
        #logging.debug("length %s", attrB.get_length())

        #logging.debug("inside %s", attrA.get_inside())
        #logging.debug("inside %s", attrB.get_inside())

        #logging.debug("angle %s", attrA.get_angle())
        #logging.debug("angle %s", attrB.get_angle())

        #logging.debug("above %s", attrA.get_above())
        #logging.debug("above %s", attrB.get_above())

        #logging.debug("overlaps %s", attrA.get_overlaps())
        #logging.debug("overlaps %s", attrB.get_overlaps())
        if attrA.get_shape() == attrB.get_shape() and\
            ...
            attrA.get_above() == attrB.get_above() and\
            attrA.get_overlaps() == attrB.get_overlaps():
            #logging.debug("yes, they are equal")
            return True
        else:
            #logging.debug("no, they are not equal")
            return False
            ...
            for attrA in list1:
                for attrB in list2:
                    if self.the_same(attrA,attrB) == True:
                        #logging.debug("removing equal attribute from our diff list")
